...
class BaiduXueshuAutomatic:
    ...
    def _init_browser(self) -> None:
        options = webdriver.ChromeOptions().add_experimental_option('excludeSwitches', ['enable-automation'])
    ...
    def run(self, wd, page_num=999, year=2018, fpath='result.txt', pos=0):
        ...
        while count <= page_num:
            ...
            try:
                ...
                print(f'谷歌浏览器被异常关闭，按ctrl+c终止终端可重新进入软件点击运行。错误信息：{e}\n')
                time.sleep(5)
                continue
            ...
            # 课程学习内部
            try:
                first_urls = self.browser.find_elements(by=By.XPATH, 
                                                       value='/html/body/div[1]/div[4]/div[3]/div[2]/div/div[@class="result sc_default_result xpath-log"]')
            except Exception as e:
                print(f'请检查软件打开的浏览器被人为（异常）终止，是的话，按ctrl+c，重新运行软件，期间不要关闭浏览器。错误信息：{e}\n')
                continue
            ...
            for ii, first_url in enumerate(first_urls):
                ...
                while status_code:
                    ...
                    second_search = re.findall('</style>.*?<tr>(.*?)</tr>', res.text, re.S)[1]
                    cite_score = re.findall(r'CiteScore:(\d+.\d+)', second_search, re.S)
                    journal_split = re.findall(r'(\d区)</td>', second_search, re.S)

                    journal_date = ''
                    fourth_search = re.findall(r'(<td.*?</td>)', second_search, re.S)
                    for i in fourth_search:
                        l = ['月', '周', 'weeks']
                        for j in l:
                            if j in i:
                                journal_date = re.findall(r'>(.*?)</td>', i, re.S)
                                break
                ...
                if cite_score and journal_split and journal_date:
                    ...
                    journal_name = f'期刊数据库未收录 Citescore:{cite_score[0]} 审稿周期:无记录 {journal_name}'
                else:
                    journal_name = f'期刊数据库未收录 无Citescore 审稿周期:无记录 {journal_name}'
...
def reg():
    ...
    try:
        ...
        print(f'{chrome_driver_path} 目录下没有chromedriver，或者版本不适配\n')
